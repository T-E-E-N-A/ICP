...
### ⏱️ Time Complexity
O(log n) Two binary searches are performed: one to find the first occurrence and one for the last. Each takes logarithmic time since the array is sorted.

### 🧠 Space Complexity
O(1) No extra space is used apart from a few variables for indices and result storage.